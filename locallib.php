...
function cr_get_my_reports($courseid, $userid, $allcourses = true) {
    ...
    if (has_capability('block/configurable_reports:managereports', $context, $userid)) {
        if ($courseid == SITEID && $allcourses) {
            $reports = $DB->get_records('block_configurable_reports', null, 'UPPER(name) ASC');
        } else {
            $reports = $DB->get_records('block_configurable_reports', array('courseid' => $courseid), 'UPPER(name) ASC');
        }
    } else {
        $reports = $DB->get_records_select('block_configurable_reports', 'ownerid = ? AND courseid = ? ORDER BY UPPER(name) ASC', array($userid, $courseid));
    }
    return $reports;
}
...
function cr_make_categories_list(&$list, &$parents, $requiredcapability = '', $excludeid = 0, $category = null, $path = '') {
    ...
    $all = $DB->get_records_sql('SELECT id, parent FROM {course_categories} ORDER BY UPPER(name) ASC');
